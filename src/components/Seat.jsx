...
const history = useHistory()
 ...
 const handleselectseat =(e)=>{
   ...
   setSelectedSeats((oldseats)=>{return [...oldseats , e]})
 ...
 const handledeleteseat =(e)=>{
  ...
  const handlebooking = ()=>{
      ...
      history.push({pathname:'/bookedtickets',state:{st:servicetax,sbt:subtotal,sbc:swachhbharatcess,kyc:krishikalyancess,t:total}})
  }
    return (
        <div className="container">
    <div className="seatcontainer">
        {
            result.map(val => val.seats.map(val=>val.seatstatus.map(
               ...
               ( val )=> {
                   ...
                   if(val.available==false){
                       ...
                       {
                           if(selectedseats[i].seatname == val.seatname)
                            index=i;
                       }
                    return (
                        <div onClick={()=>{
                         ...
                         }} className="seatbox2">{val.seatname}</div>);
                   }
                   return (
               <div onClick={()=>{
                   val.available=false;
                   handleselectseat(val)
                }} className="seatbox1">{val.seatname}</div>);
            }
